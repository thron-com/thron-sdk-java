package it.newvision.nvp.xpackager.services.model.repository
import _root_.java.util._ 
import _root_.java.lang.{Integer,Boolean,Long,Double,Float,Short}  
//#SWG#import com.wordnik.swagger.annotations._ 
import _root_.scala.beans.BeanProperty 
import javax.xml.bind.annotation._ 

/* ************************
*  GENERATED CLASS
*  DO NOT APPLY ANY CHANGES
****************************/
@XmlRootElement(name="MSearchCriteriaExtended") 
@XmlType(name="MSearchCriteriaExtended")
//#SWG#@ApiModel(description = """""")
class MSearchCriteriaExtended extends MSearchCriteria with Serializable  {

	//#SWG#@ApiModelProperty(value = """""")
	@BeanProperty 
	var clientIds: List[String] = new ArrayList[String]
	def withclientIds(p:List[String]):MSearchCriteriaExtended ={ 	this.clientIds = p; 	this }

}