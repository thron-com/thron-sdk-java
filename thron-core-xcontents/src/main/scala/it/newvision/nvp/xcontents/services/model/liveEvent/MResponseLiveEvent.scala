...
class MResponseLiveEvent extends Serializable {
	...
	def withresultCode(p:MEContent):MResponseLiveEvent ={ 	this.resultCode = p; 	this }

	//#SWG#@ApiModelProperty(value = """""" ,required = true)
	@BeanProperty 
	var ssoCode: MESSO =_
	def withssoCode(p:MESSO):MResponseLiveEvent ={ 	this.ssoCode = p; 	this }

	//#SWG#@ApiModelProperty(value = """""" ,required = true)
	@BeanProperty 
	var errorDescription: String =_
	def witherrorDescription(p:String):MResponseLiveEvent ={ 	this.errorDescription = p; 	this }
