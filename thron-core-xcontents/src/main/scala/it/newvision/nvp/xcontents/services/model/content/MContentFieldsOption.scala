...
class MContentFieldsOption extends Serializable {
	...
	/**
	 * if true, source will be included in the response.
	 */
	//#SWG#@ApiModelProperty(value = """if true, source will be included in the response.""" ,required = true)
	@BeanProperty
	var returnSource: Boolean  = false
	def withreturnSource(p:Boolean):this.type ={ 	this.returnSource = p; 	this }
