...
trait JClient extends it.newvision.nvp.core.libraries.restserver.BaseResource {
	...
	*/
	...
	//#SWG#@ApiOperation(value = "/delivery", notes = """Update delivery settings of client.""", response = classOf[MResponseDelivery])
			//#SWG#@ApiResponses(value=Array(new ApiResponse(code=200, message="OK"),new ApiResponse(code=400, message="Invalid Arguments"),new ApiResponse(code=418, message="Exception"),new ApiResponse(code=403, message="Access Denied/Session Expired"), new ApiResponse(code=404, message="Not Found"), new ApiResponse(code=307, message="Temporary redirect")))
	def delivery(//#SWG#@ApiParam(name = "X-TOKENID", value = "session token", required=false)
	@HeaderParam("X-TOKENID")
	tokenId: String, 
			param: MClientdeliveryReq):Response /*returnType = MResponseDelivery*/ = {
		import it.newvision.nvp.core.libraries.restserver.PRestHelper
		import it.newvision.core.dictionary.exceptions.WebApplicationException
		try{
			val resp = this.__delivery(tokenId,param)
			PRestHelper.responseForPOST(resp, this._postCacheControl,this.capability_delivery)    
		}catch{
	      case e:WebApplicationException =>
	        throw new WebApplicationException(e,this.capability_delivery)
	    }
	} 
	...
	def delivery_2(@HeaderParam("X-TOKENID") tokenId_h: String,
		...
		val cc = this.cachemap.getOrElse("delivery",this._getCacheControl) 
		try{
			val resp = this.__delivery(
			PRestHelper.getTokenId(tokenId_q, tokenId_h)
			,PRestHelper.bindRequest[MClientdeliveryReq](param_q)	
		    )
	      PRestHelper.responseForGET(resp, cc, callback_q,this.capability_delivery)
	    }catch{
	      case e:WebApplicationException=>
	        if(StringUtils.isBlank(callback_q)) throw e
	        PRestHelper.responseAsException(e.getResponse, this._getCacheControl, callback_q,this.capability_delivery)
	    }
	}

	/** ABSTRACT METHOD TO IMPLEMENT */ 
	 protected def __delivery(tokenId: String, param: MClientdeliveryReq) :MResponseDelivery
	/** ABSTRACT METHOD. IMPLEMENT USING THE RIGHT CAPABILITY NAME */ 
	protected def capability_delivery: String
