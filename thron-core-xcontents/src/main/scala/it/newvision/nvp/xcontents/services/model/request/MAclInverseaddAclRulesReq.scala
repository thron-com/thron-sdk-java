...
class MAclInverseaddAclRulesReq extends Serializable {
	...
	def withclientId(p:String):MAclInverseaddAclRulesReq ={ 	this.clientId = p; 	this }

	//#SWG#@ApiModelProperty(value = """""", required = true)
	@BeanProperty
	var rules: MAclRuleInverseItems =_
	def withrules(p:MAclRuleInverseItems):MAclInverseaddAclRulesReq ={ 	this.rules = p; 	this }
