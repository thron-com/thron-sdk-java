 ...
 */
@Path("/liveeventadmin")
//#SWG#@Api(value = "/liveeventadmin", description = """Used to create specific live Events contents. 
//#SWGNL#<b>
...
trait JLiveEventAdmin extends it.newvision.nvp.core.libraries.restserver.BaseResource {
	...
	*/
	...
	//#SWG#@ApiOperation(value = "/create", notes = """create a new live event""", response = classOf[MResponseLiveEventCreate])
	...
	tokenId: String, 
			param: MLiveEventAdmincreateReq):Response /*returnType = MResponseLiveEventCreate*/ = {
	...
	def create_2(@HeaderParam("X-TOKENID") tokenId_h: String,
			...
			@QueryParam("callback") callback_q: String):Response /*returnType = MResponseLiveEventCreate*/ = { 
		import it.newvision.nvp.core.libraries.restserver.PRestHelper
		import it.newvision.core.dictionary.exceptions.WebApplicationException
		...
		try{
			...
			,PRestHelper.bindRequest[MLiveEventAdmincreateReq](param_q)	
		    )
	...
	/** ABSTRACT METHOD TO IMPLEMENT */ 
	 protected def __create(tokenId: String, param: MLiveEventAdmincreateReq) :MResponseLiveEventCreate
	...
	*/
	...
	@Produces(Array(MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML))
	@Consumes(Array(MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML))
	//#SWG#@ApiOperation(value = "/update", notes = """update a live event """, response = classOf[MResponseLiveEventUpdate])
			//#SWG#@ApiResponses(value=Array(new ApiResponse(code=200, message="OK"),new ApiResponse(code=400, message="Invalid Arguments"),new ApiResponse(code=418, message="Exception"),new ApiResponse(code=403, message="Access Denied/Session Expired"), new ApiResponse(code=404, message="Not Found"), new ApiResponse(code=307, message="Temporary redirect")))
	def update(//#SWG#@ApiParam(name = "X-TOKENID", value = "session token", required=false)
	@HeaderParam("X-TOKENID")
	tokenId: String, 
			param: MLiveEventAdminupdateReq):Response /*returnType = MResponseLiveEventUpdate*/ = {
		import it.newvision.nvp.core.libraries.restserver.PRestHelper
		import it.newvision.core.dictionary.exceptions.WebApplicationException
		try{
	...
	def update_2(@HeaderParam("X-TOKENID") tokenId_h: String,
		...
		try{
			...
			PRestHelper.getTokenId(tokenId_q, tokenId_h)
			,PRestHelper.bindRequest[MLiveEventAdminupdateReq](param_q)	
		    )
	...
	/** ABSTRACT METHOD TO IMPLEMENT */ 
	 protected def __update(tokenId: String, param: MLiveEventAdminupdateReq) :MResponseLiveEventUpdate
	/** ABSTRACT METHOD. IMPLEMENT USING THE RIGHT CAPABILITY NAME */ 
