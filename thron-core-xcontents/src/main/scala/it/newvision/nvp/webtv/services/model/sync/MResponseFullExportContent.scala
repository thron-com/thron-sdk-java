...
class MResponseFullExportContent extends MResponseContent with Serializable  {
	...
	@BeanProperty 
	var items: List[MSyncFullExportContentResult] = new ArrayList[MSyncFullExportContentResult]
	def withitems(p:List[MSyncFullExportContentResult]):this.type ={ 	this.items = p; 	this }
