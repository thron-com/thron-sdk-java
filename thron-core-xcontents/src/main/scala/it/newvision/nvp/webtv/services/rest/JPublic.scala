...
trait JPublic extends it.newvision.nvp.core.libraries.restserver.BaseResource {
	...
	*/
	 ...
	 * Optional. Id or prettyId of content that will be returned if the original document request is
	 * missing or the access is denied.
	 * @param placeholderPkey : String
	 * Optional. Pkey of placeholder content. If not present, the token id is used for validation.
	 * @return java.io.File
	*/
	@GET
	...
	@QueryParam("dpr")
	dpr: Integer, 
			//#SWG#@ApiParam(value = """Optional. Id or prettyId of content that will be returned if the original document request is missing or the access is denied.""")
	@QueryParam("placeholderId")
	placeholderId: String, 
			//#SWG#@ApiParam(value = """Optional. Pkey of placeholder content. If not present, the token id is used for validation.""")
	@QueryParam("placeholderPkey")
	placeholderPkey: String,
			//#SWG#@ApiParam(value = "Optional",required=false,access="internal")
			@QueryParam("callback") callback_q: String
			,
			//#SWG#@ApiParam(value = "Deprecated. If required, use the X-TOKENID header parameter.",required=false,access="internal")
		...
		try{	
			val resp = this.__image(PRestHelper.getTokenId(tokenId_q, tokenId),clientId,id,pkey,preset,divArea,prettyName,lcid,scalemode,quality,cropmode,cropx,cropy,cropw,croph,adjustcrop,fill,fillcolor,format,enhance,dpr,placeholderId,placeholderPkey)
	 ...
	 * @param placeholderId : String
	 * Optional. Id or prettyId of content that will be returned if the original document request is
	 * missing or the access is denied. The content must have type IMAGE
	 * @param placeholderPkey : String
	 * Optional. Pkey of placeholder content. If not present, the token id is used for validation.
	 * @return java.io.File
	*/
	...
	@QueryParam("dpr")
	dpr: Integer, 
			//#SWG#@ApiParam(value = """Optional. Id or prettyId of content that will be returned if the original document request is missing or the access is denied. The content must have type IMAGE""")
	@QueryParam("placeholderId")
	placeholderId: String, 
			//#SWG#@ApiParam(value = """Optional. Pkey of placeholder content. If not present, the token id is used for validation.""")
	@QueryParam("placeholderPkey")
	placeholderPkey: String,
		...
		try{	
			val resp = this.__thumbnail(PRestHelper.getTokenId(tokenId_q, tokenId),clientId,id,pkey,preset,divArea,prettyName,lcid,scalemode,quality,cropmode,cropx,cropy,cropw,croph,adjustcrop,fill,fillcolor,format,enhance,dpr,placeholderId,placeholderPkey)
	...
	/**
	 * This service provides the thumbnail of a product with the desired resolution and quality.
	 * The path parameter 'id' accept a product id or a product unique alias.
	 * 
	 * Attention: this service makes use of cache control to ensure best performance.
	 * 
	 * HTTP status codes:
	 ...
	 * 	<li>500: generic error,</li>
	 * 	<li>307: redirects to resulting product thumbnail,</li>
	 * 	<li>200: ok.</li>
	 * </ul>
	 * 
	 * If no thumbnail is available, a default fallback image will be provided.
	 ...
	 * ClientId
	 * @param id : String
	 * The product id, or unique alias.
	 * @param pkey : String
	 * The key to access of the product thumbnail.
	 * @param preset : String
	 * RTIE preset value. Default value is "std".
	 * @param divArea : String
	 * The desired WidthxHeight of the resulting image. If higher than the original thumbnail resolution,
	 ...
	 * Optional. Device Pixel Ration, available values are [1..1000]
	 * @param placeholderId : String
	 * Optional. Id or prettyId of the content that will be returned if the original product request is
	 * missing or the access is denied.
	...
	*/
	@GET
	@Path("/productThumbnail/{clientId}/{id}/{pkey}/{preset}/{divArea}/{prettyName}")
	@Produces(Array(MediaType.WILDCARD,"application/x-javascript"))
	//#SWG#@ApiOperation(value = "/productThumbnail", notes = """This service provides the thumbnail of a product with the desired resolution and quality.
	//#SWGNL#The path parameter 'id' accept a product id or a product unique alias.
	//#SWGNL#
	//#SWGNL#Attention: this service makes use of cache control to ensure best performance.
	//#SWGNL#
	//#SWGNL#HTTP status codes: 
	//#SWGNL#<ul>
	//#SWGNL#	<li>400: invalid arguments,</li>
	//#SWGNL#	<li>404: product not found,</li>
	//#SWGNL#	<li>500: generic error,</li>
	//#SWGNL#	<li>307: redirects to resulting product thumbnail,</li>
	//#SWGNL#	<li>200: ok.</li>
	//#SWGNL#</ul>
	//#SWGNL#
	//#SWGNL#If no thumbnail is available, a default fallback image will be provided. 
	//#SWGNL#This service is public: authentication token is not required (X-TOKENID).""", response = classOf[java.io.File])
			//#SWG#@ApiResponses(value=Array(new ApiResponse(code=200, message="OK"),new ApiResponse(code=400, message="Invalid Arguments"),new ApiResponse(code=418, message="Exception"),new ApiResponse(code=403, message="Access Denied/Session Expired"), new ApiResponse(code=404, message="Not Found"), new ApiResponse(code=307, message="Temporary redirect")))
	def productThumbnail(//#SWG#@ApiParam(name = "X-TOKENID", value = "session token", required=false)
	@HeaderParam("X-TOKENID")
	tokenId: String, 
			//#SWG#@ApiParam(value = """ClientId""")
	@PathParam("clientId")
	clientId: String, 
			//#SWG#@ApiParam(value = """The product id, or unique alias.""")
	@PathParam("id")
	id: String, 
			//#SWG#@ApiParam(value = """The key to access of the product thumbnail.""")
	@PathParam("pkey")
	pkey: String, 
			//#SWG#@ApiParam(value = """RTIE preset value. Default value is "std".""")
	@PathParam("preset")
	...
	@QueryParam("dpr")
	dpr: Integer, 
			//#SWG#@ApiParam(value = """Optional. Id or prettyId of the content that will be returned if the original product request is missing or the access is denied. 
	//#SWGNL#""")
	@QueryParam("placeholderId")
	placeholderId: String, 
			//#SWG#@ApiParam(value = """Optional. Pkey of placeholder content. If not present, the token id is used for validation.""")
	...
	placeholderPkey: String,
		...
		import org.apache.commons.lang.StringUtils
		//get the cache control specific for this service
		val cc = this.cachemap("productThumbnail") 
		try{	
			val resp = this.__productThumbnail(PRestHelper.getTokenId(tokenId_q, tokenId),clientId,id,pkey,preset,divArea,prettyName,scalemode,quality,fill,fillcolor,format,dpr,placeholderId,placeholderPkey)
		
			PRestHelper.responseForGET(resp, cc, callback_q,this.capability_productThumbnail)
	    }catch{
	      case e:WebApplicationException=>
	        if(StringUtils.isBlank(callback_q)) throw e
	        PRestHelper.responseAsException(e.getResponse, this._getCacheControl, callback_q,this.capability_productThumbnail)
	    }
	}
	...
	/** ABSTRACT METHOD TO IMPLEMENT */ 
	 protected def __productThumbnail(tokenId: String, clientId: String, id: String, pkey: String, preset: String, divArea: String, prettyName: String, scalemode: String, quality: Integer, fill: String, fillcolor: String, format: String, dpr: Integer, placeholderId: String, placeholderPkey: String) :java.io.File
	/** ABSTRACT METHOD. IMPLEMENT USING THE RIGHT CAPABILITY NAME */ 
	protected def capability_productThumbnail: String
