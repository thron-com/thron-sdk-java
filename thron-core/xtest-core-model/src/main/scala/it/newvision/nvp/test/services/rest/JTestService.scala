...
//#SWGNL#<ul>
//#SWGNL#	<li>WADL REST service (latest version): http://clientId-view.4me.it/api/xtest/resources/application.wadl  </li>
//#SWGNL#</ul>""")
trait JTestService extends it.newvision.nvp.core.libraries.restserver.BaseResource {
	...
	*/
	...
	clientId: String,
		...
		try{	
			val resp = this.__listGet(tokenId,clientId)
	...
	*/
	...
	pfloat: Float,
		...
		try{	
			val resp = this.__get(tokenId,clientId,fakeResponse,pstring,penum,pdate,pint,pbool,plong,pdouble,pfloat)
