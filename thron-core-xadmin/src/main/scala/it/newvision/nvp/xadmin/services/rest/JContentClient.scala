...
class JContentClient(val resourceEndpoint:String) {
	...
	*/
	def generateDownloadToken(tokenId: String, 
			clientId: String, 
			param: MContentgenerateDownloadTokenReq)(implicit _fwdHeaders:Option[scala.collection.Map[String,String]]=None):MResponseGenerateDownloadToken ={
	
		  import scala.collection.JavaConversions._
		  try{
			val webResource = JContentClient.client.resource(this.resourceEndpoint)
			val response : MResponseGenerateDownloadToken = if(this.resourceEndpoint == ""){
			
				new MResponseGenerateDownloadToken()
			
			}else{	
				val mediaType = javax.ws.rs.core.MediaType.APPLICATION_XML	
				var wbuilder = webResource
					.path("content/generateDownloadToken")
					.path(clientId.toString)
					.accept(javax.ws.rs.core.MediaType.APPLICATION_XML)		
					.`type`(mediaType)
					.header("X-TOKENID",tokenId)
				...
				wbuilder.post(classOf[MResponseGenerateDownloadToken],param)
			...
			case e : com.sun.jersey.api.client.UniformInterfaceException =>
				val response = e.getResponse
				if(response.getStatus == 418) {
				  response.getEntity(classOf[MResponseGenerateDownloadToken])
				}
				else {
				  throw e
				}
