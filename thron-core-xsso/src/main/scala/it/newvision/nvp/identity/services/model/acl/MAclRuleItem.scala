...
class MAclRuleItem extends Serializable {
	...
	def withsourceObjId(p:String):MAclRuleItem ={ 	this.sourceObjId = p; 	this }

	//#SWG#@ApiModelProperty(value = """""" ,required = true)
	@BeanProperty 
	var sourceObjClass: MEObjClass =_
	def withsourceObjClass(p:MEObjClass):MAclRuleItem ={ 	this.sourceObjClass = p; 	this }

	//#SWG#@ApiModelProperty(value = """""" ,required = true)
	@BeanProperty 
	var targetAcl: MAclRule =_
	def withtargetAcl(p:MAclRule):MAclRuleItem ={ 	this.targetAcl = p; 	this }
